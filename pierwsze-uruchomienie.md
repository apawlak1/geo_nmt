...
`geo_nmt` automatyzuje pozyskiwanie **numerycznego modelu terenu (NMT)** z Geoportalu (GUGiK/PZGiK) dla
wybranego powiatu. Zamiast ręcznie przeglądać skorowidze, pobierać pojedyncze arkusze i sklejać je w
jedną całość, jedno uruchomienie skryptu robi to wszystko za Ciebie i — zależnie od ustawień — oddaje
gotowy wynik w jednej z trzech postaci:
...
Ty tylko wskazujesz powiat i lata.
...
- Żeby uruchomić program ponownie później (np. po pojawieniu się nowych danych w Geoportalu), po prostu
  odpal `python geo_nmt.py` jeszcze raz — program sam rozpozna, co już ma, i dociągnie tylko brakujące dane.
- Szczegółowy opis wszystkich możliwości configu, struktury wyników i pracy z ArcGIS/QGIS jest w
  `README.md` i `INSTRUKCJA_OBSLUGI.md`.
